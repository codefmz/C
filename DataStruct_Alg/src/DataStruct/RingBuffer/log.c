...
int32_t LogInfo(int32_t t_iType, const char* t_pFilenamePart, const char* t_pLog, int32_t t_iLine, const char* t_pFun, const char* t_pFile)
{
    ...
    if (t_iType == LOG_ERROR_LEVEL) {
        snprintf(szFilename, 128, "./log/error_%s.log", GetCurShortDateStr());
        snprintf(szLog, 2048, "[%s][ERROR]%s:%d[%s]:%s\n", GetCurDateTimeStr(), t_pFile, t_iLine, t_pFun, t_pLog);
    } else if (t_iType == LOG_INFO_LEVEL) {
        snprintf(szFilename, 128, "./log/info_%s.log", GetCurShortDateStr());
